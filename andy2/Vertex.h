...
struct vertex {
  ...
  CVector pv;
  CVector n;
  vertex(CVector pb, CVector nn, CPoint p1, double uu, double vv)
      : pv(pb), n(nn), p(p1), u(uu), v(vv) {};
...
public:
  ...
  CVertex(CVector vr)
      : constrained(false), v(vr), adje(0), adjt(0), select(false),
        visible(false), sit(-1), checked(false) {
    adjE.clear();
  };
  CVertex(CVector vr, float u, float v, int i = -1)
      : constrained(false), visible(false), v(vr), un(u), vn(v), sit(i),
        adje(0), adjt(0), select(false), checked(false) {
    adjE.clear();
  };
  CVertex(CMesh *m = NULL)
      : constrained(false), mesh(m), adje(0), adjt(0), select(false),
        visible(false), sit(-1), checked(false) {
    adjE.clear();
  };
  void operator+=(CVertex vert) { v += vert.v; }
