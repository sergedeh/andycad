...
void CCAM::Rotate(int sens, CVector prevpoint, CVector postpoint, bool b) {
  ...
  double alphab =
      v.argu(postpoint.camxyz()) - v.argu(prevpoint.camxyz()); //-argu(piv);
...
float CCAM::getzoom() { return CCAM::zoom_r; }
...
CVector CCAM::getcamor() { return camor; }

CVector CCAM::gettarget() { return target; }
...
float CCAM::getfov() { return fov; }

void CCAM::setfarplane(float f) { CVector::f = f; }

void CCAM::setnearplane(float n) { CVector::n = n; }

float CCAM::getfarplane() { return CVector::f; }

float CCAM::getnearplane() { return CVector::n; }
...
void CCAM::init(CFigure *f) {
  ...
  // This vector is to shift the screen to start at the bottom left of the
  // bounding box The x represent the bottom of the screen while the y is the
  // left side (check the CVEctor content
