...
//	result=new double[ab*(p+n+3)];
	...
	int iterp = p + n + 1 > B.size() - 1 ? B.size() - 1 : p + n + 1;
	for(int iter=0;iter<=iterp;iter++)
	{
		if (B[0]==B[iter])
		{
			bmult++;
		...
		if ((p+n+1 < B.size()) && (B[iter]==B[iterp]))
		{
			emult++;
		}
	}
	...
	for( int iter=0;iter<iterp;iter++)
	{
		...
		if ((p + n + 1 < B.size()) && (u==B[p+n+1])&&(iter==n))
		{
			if (emult=p+1)
			{
			result[ab*iter]=1;
	...
	for (int j=1;j<=p;j++)
	{
		...
		for(int iter=0;iter<n+p+1-j;iter++)
		{
			...
			if((iter + j < B.size()) && !(B[iter+j]==B[iter]))
			{
			...
			if((iter + j + 1 < B.size()) && !(B[iter+j+1]==B[iter+1]))
			{
...
void CBezier::Bohms(double t, int p,vector<double>& U,vector<CVector>& P,int &n)
{
	vector<CVector> Pv;
	int k=0;
	int m = n + p + 1 > U.size() - 1 ? U.size() - 1 : n + p + 1;;
	vector<double> Uc;
...
*/
	...
	Uc.push_back(t);
	int uu = U.size();
	for(iter=k+2;iter<=m+1  && iter < uu;iter++)
	{
	...
	U.resize(U.size()+1);
	int uu1 = Uc.size();
	for(iter=0;iter<=n+p+1 && iter < uu1;iter++)
	{
		U[iter]=Uc[iter];
...
void CBezier::copy(const CFigure *fig)
{
	...
	for(int i=0; i<bg->Kn.size();i++)
	{
		Kn[i]=bg->Kn[i];
	}
	Kn1.resize(bg->Kn1.size());
	for(int i=0; i<bg->Kn1.size();i++)
	{
		Kn1[i]=bg->Kn1[i];
	}
	Kn2.resize(bg->Kn2.size());
	for(int i=0; i<bg->Kn2.size();i++)
	{
		Kn2[i]=bg->Kn2[i];
	}
	Para.resize(bg->Para.size());
	for(int i=0; i<bg->Para.size();i++)
	{
		Para[i]=bg->Para[i];
	}
	Para1.resize(bg->Para1.size());
	for(int i=0; i<bg->Para1.size();i++)
	{
		Para1[i]=bg->Para1[i];
	}
	Para2.resize(bg->Para2.size());
	for(int i=0; i<bg->Para2.size();i++)
	{
		Para2[i]=bg->Para2[i];
	}
...
void CBezier::revolsurface(vector<CBezier*> fg, int l,bool simple)
{
	...
	for(int i=0;i<fg[1]->Para.size();i++)
	{
		Para[i]=fg[1]->Para[i];
	}
	deg1=2;
...
//				  vertex->display(hdc);
	   ...
	   int el=mesh.Ecount();
	   int i=0;
	   for(int kiter=0;kiter<=ncpw;kiter++)
	   {
	   for(int jiter=0;jiter<ncpv;jiter++)
	   {
		 for(int iter=0;iter<ncpu;iter++)
