...
bool CDraw::add(CString figure, bool fPrevLine, CDC *hdc, CVector point) {
  ...
  if (figure == "Line") {
    ...
    endDraw = pRectangle.draw(fPrevLine, hdc, point);
    return true;
  } else if (figure == "3DotCircle") {
    endDraw = pCircle.Draw3DotCircle(fPrevLine, hdc, point);
    return true;
  } else if (figure == "2DotCircle") {
    ...
    return true;
  } else if ((figure == "multline") || (figure == "leader") ||
             (figure == "DimA") || (figure == "DimF") || (figure == "DimV") ||
             (figure == "DimH") || (figure == "DimD") || (figure == "DimR")) {
    //		if(candraw)
    endDraw = ((CDim *)fig[fit - 1])->draw(fPrevLine, hdc, point);
    curr_fig = fig[fit - 1];
    return true;
  } else if ((figure == "DimL") || (figure == "DimLH") || (figure == "DimLV")) {
    //		if(candraw)
...
void CDraw::display(CDC *rhdc, CDC *hdc) {
  ...
  try {
    ...
    int j = 0;
    for (vector<FLAYER>::iterator it = veclayer.begin(); it < veclayer.end();
         it++) {
      if (!it->clear) {

        for (int i = 0; i < fit; i++) {
          if (fig[i]->getLayer().name == it->name) {
            if (!sc_box.intersectAABB2(fig[i]->box) /*getbox(hdc))*/)
              continue;
...
void CDraw::Create(CDC *hdc, CString figure, CVector point) {
  ...
  if (figure == "Line") {
  ...
  } else if ((figure == "DimD") || (figure == "DimR")) {
    ...
    if (!valid) {
      ...
      for (int i = 0; i < fit - 1; i++) {
        vector<CVector> test;
        if ((fig[i]->getName() == cerc.getName()) &&
            (fig[i]->inRegion(point))) {
          ...
          if (test[0] != test[2]) {
            ...
            break;
             ...
             (figure == "DimF") || (figure == "DimV") || (figure == "DimH")) {
    if (!valid) {
      string scount;
      CString f;
      f.Format(_T("%d"), count);
    ...
    if (nbc == 0) {
      ...
      for (int i = 0; i < fit; i++) {
        ...
        if ((fig[i]->getName() == pL.getName()) ||
            (fig[i]->getName() == pR.getName()) ||
            (fig[i]->getName() == pQ.getName())) {
          test = fig[i]->getSelectvec(point);
          if (test.size() != 0) {
            if (test[0] != test[2]) {
              /*vhan[sfit]=fig[i]->gethandle();*/ vhan.push_back(
                  fig[i]->gethandle());
              sfit++;
              IA = new CDimA;
    ...
    pRectangle.setPtsBegin(point);
      ...
      if ((figure == "Circle") || (figure == "2DotCircle") ||
          (figure == "3DotCircle") || (figure == "Ellipse")) {
    pCircle.setPtsBegin(point);
  } else if ((figure == "Arc") || (figure == "2DotArc") ||
             (figure == "3DotArc") || (figure == "3DotArc2")) {
...
void CDraw::finaladd(CDC *hdc, CString figure, CVector pos2, CString option) {
  ...
  if (endDraw) {
    ...
    if (figure == "Line") {
      ...
      if (pLine.getCharge()) {
        ...
        ftp->copy(&pLine);
        /*vhan[sfit]=count;*/ vhan.push_back(scount);
        /*fig=(CFigure**) realloc(fig,m+sizeof(CFigure*));*/ fig.resize(
            fig.size() + 1);
        ////m=_msize(fig);
        fig[fit] = ftp;
        forundo(fit, "create line");
    ...
    if (figure == "QLine") {
      ...
      if (valid) {
        ...
        forundo(fit - 1, "create dima");
               ...
               (figure == "DimV") || (figure == "DimH") || (figure == "DimF")) {
      if (!valid) {
    ...
    } else if ((figure == "DimL") || (figure == "DimLH") ||
               (figure == "DimLV")) {
      if (!valid) {
      ...
      pSelect.draw(hdc);
      candraw = false;
    } else if ((figure == "Circle") || (figure == "2DotCircle")) {
  ...
  if ((figure == "3DotArc") || (figure == "3DotArc2") || (figure == "Arc")) {
    if (pArc.getCharge()) {
      if (figure == "3DotArc")
        pArc.setType("3DotArc");
      if (figure == "3DotArc2")
        pArc.setType("3DotArc2");
      if (figure == "Arc")
        pArc.setType("Arc");
      pArc.addtovec(hdc);
      CCercle *ftp = new CCercle;
...
bool CDraw::select(bool spoint, int multiselect, CVector point, CDC *hdc) {
  ...
  for (i = 0; i < fit; i++) {
    if ((fig[i]->getLayer()).on) {
      ...
      {
        if (!sc_box.intersectAABB2(fig[i]->box))
          continue;
        if (spoint) {
...
bool CDraw::translate(CWnd *wnd, CDC *hdc, CVector PrevPos, CVector PostPos,
                      int key) {
  bool sel = false;
  int g = vs.size();
  CAABB box;
  ...
  for (int i = 0; i < g; i++) {
    ...
    sel = true;
    if (key == 0)
      vs[i]->translate(wnd, hdc, PrevPos,
                       PostPos); // mouse.getPrevPos(),mouse.getPostPos());
    else {
      vs[i]->keytranslate(wnd, hdc,
                          key); // mouse.getPrevPos(),mouse.getPostPos());
    }
    change = true;
  }
  getBinfo(PostPos);
...
void CDraw::translatecam(CVector v) { cam.Translate(v); }
...
bool CDraw::closecurve(CVector point, CDC *hdc) {
  ...
  //	{
                  if
  ((fig[i]->inselectedstate())&&(fig[i]->getName()!=textc.getName()))
                  {
  ...
  for (int i = 0; i < l; i++) {
    //			if(fig[i]->transselectedstate())
    {
      ...
      if ((vs[i]->getName() == pL.getName()) ||
        ...
        vs[i]->setInit();
        b = true;
      } else if ((vs[i]->getName() == pC.getName()) ||
...
bool CDraw::getselect() { return Bminus; }
...
void CDraw::pastearray(CDC *hdc, bool of, int nbrow, int nbcol, double distx,
                       double disty, int nblevel, double lspacing) {
  ...
  for (int i = 0; i < pk; i++) {
    ...
    {
      arr = vs[i]->translatepArray(of, nbrow, nbcol, distx, disty, nblevel,
                                   lspacing);
      vs[i]->setMultselect(false);
...
void CDraw::save(CDC *hdc, CFile &file) {
  ...
  for (vector<FLAYER>::iterator iter = veclayer.begin(); iter < veclayer.end();
       iter++) {
...
void CDraw::copyRSparam2(CDC *hdc, double angle, int nbcopy, int nblevel,
                         int dlevel, bool align, CVector v) {
  int pk = vs.size();
...
void CDraw::copyRSparam(CDC *hdc, double angle, int nbcopy, int nblevel,
                        int dlevel, bool align) {
  ...
  for (int i = 0; i < fit; i++) {
    ...
    if (fig[i]->inselectedstate()) {
      if (fig[i]->getName() != pLibs.getName()) {
        ...
        for (fiter = arr->begin(); fiter < arr->end(); fiter++) {
          ...
          if ((fig[i]->getName() == pLine.getName()) ||
            ...
            *ftp = pLine;
            ...
            fig[pk] = ftp;
            pk++;
          } else if (fig[i]->getName() == pCircle.getName()) {
            /*	CCercle pC;
            pC=*(CCercle*)fig[i];
            ...
            fig[pk] = ftp;
            pk++;
          } else if (fig[i]->getName() == pBezier->getName()) {
            ...
            fig[pk] = ftp;
            pk++;
          } else if (fig[i]->getName() == textc.getName()) {
        ...
        lib = pInter.RotateCopyP(hdc, fig[i]->getRotateCenter(), center, align,
                                 (CLibS *)fig[i]);
        int k = lib[0].x;
...
void CDraw::setEndPoly(bool end) { menuselect = end; }
...
void CDraw::setLcolor(CDC *hdc, COLORREF color, COLORREF bcolor) {
  ...
  for (int i = 0; i < j; i++) {
    ...
    } else if (vs[i]->getName() == textc.getName()) {
...
void CDraw::undo(const string& cmd,bool rdo) {
...
CRect CDraw::grid(CDC *rhdc, CDC *hdc, CSize cp) {
  ...
  CVector v0(k.getcamor()), vx(100, 0, 0, 0), vy(0, 100, 0, 0), v1,
      vz(0, 0, 100, 0);
  ...
  //	displaygrid(hdc,10);
  CVector t0(v1.getcamor()), tx(v1.getcamx()), ty(v1.getcamy()),
      tz(v1.getcamz());
...
void CDraw::group(CDC *hdc) {
  ...
  if (l > 1) {
    ...
    for (i = 0; i < l; i++) {
      ...
      {
        CFigure *ftp =
            vs[i]->translatep(vs[i]->getCenter(), vs[i]->getCenter());
        lib->add(ftp);
        forundo(i, "delete");
      }
    }
...
void CDraw::group(CLibS &lib) {
  ...
  for (int i = 0; i < j; i++) {
    ...
    if (vs[i]->getName() == pL.getName()) {
      ...
      *ftp = pL;
      lib.add(ftp);
    } else if (vs[i]->getName() == pR.getName()) {
      pR = *(CRectangle *)vs[i];
      pR.addvec(*arr, *(arr + 1));
      delete[] arr;
      CRectangle *ftp = new CRectangle;
      *ftp = pR;
      ...
      ftp->copy(pC);
      lib.add(ftp);
    } else if (vs[i]->getName() == pB.getName()) {
      ...
      *ftp = textc;
      lib.add(ftp);
    } else if (vs[i]->getName() == image.getName()) {
      image = *(CImage *)vs[i];
      ...
      *ftp = image;
      lib.add(ftp);
    } else if (vs[i]->getName() == lib.getName()) {
      pli.Copy(*(CLibS *)vs[i]);
...
void CDraw::getBinfo(CVector point) {
  ...
  for (int i = 0; i < g; i++) {
    ...
    for (vector<CObserver *>::iterator it = observers.begin();
         it != observers.end(); it++) {
...
CRect CDraw::getgrid(CSize p) {
  CVector v1(-(p.cx / 2), -(p.cy / 2)), v2((p.cx / 2), -(p.cy / 2)),
      v3((p.cx / 2), (p.cy / 2)),
      v4(-(p.cx / 2),
         (p.cy / 2)); //-425,550),v2(425,550),v3(425,-550),v4(-425,-550);
                      // CVector v1(0,0),v3(p.cx,p.cy);
  return CRect(v1, v3);
}

CPoint CDraw::getCenter() { return cp; }
...
CVector CDraw::getMouse(CVector v) { return v; }
...
void CDraw::saveBMP(CDC *hdc, CBitmap *bitmap, CFile &file) {
  ...
  /*	HGLOBAL hDib;
          ...
          file.Write((LPSTR)lpBi,
     bi.biSize+dwSizeCT*sizeof(RGBQUAD)+bi.biSizeImage);
          ::GlobalUnlock(hDib);
          ::GlobalFree(hDib);
          */
  //	delete hmdc;
...
void CDraw::AddBlock2(string &name, string &layer, string &handle, CVector vo) {
  ...
  bool find = false;
  for (vector<CLibS *>::iterator it1 = blocks.begin(); it1 != blocks.end();
       it1++) {
...
void CDraw::AddBlock3(string &name, float rot_angle, CVector vscale, CVector vo,
                      CVector col_row_count, CVector col_row_spacing) {
  int ffit = vblocks.size();
  int j0 = 0;
  CFigure *fg;
  for (int i = 0; i < ffit; i++) {
    if (vblocks[i]->getLibName().c_str() == name) {
      ...
      pastearray(NULL, false, col_row_count.x, col_row_count.y,
                 col_row_spacing.x, col_row_spacing.y, 1, 0);
    }
  }
}
...
void CDraw::Insertlib(CDC *hdc,const CString& f,const CString& l,CVector pos) {
  CLibS *pLibs;
  CInter pInter;
  CFigure *fg;
  int ffit = vblocks.size();
      ...
      if (version == 0) {
        ...
        for (int i = 0; i < ffit; i++) {
          ...
          if (m == l) {
            ...
            /*						CFigure*
               h=pInter.paste(f->getCenter(),pos,f); delete f; h->draw(hdc);
                                                            SELECT s=full;
                                                            h->setselectmode(s);
                                                            h->setclick(true);
...
void CDraw::setsnap(bool b) { snap = b; }
...
void CDraw::addlayer(CListCtrl &tcl, CString ltext) {
  LVITEM it;
  it.mask = LVIF_TEXT | LVIF_IMAGE | LVIF_STATE;
...
void CDraw::removelayer(const CString &txt) {
  for (vector<FLAYER>::iterator it = veclayer.begin(); it < veclayer.end();
       it++) {
    if (it->name == txt) {
      for (int i = 0; i < fit; i++) {
...
void CDraw::uplayer(const CString &txt) {
  FLAYER fl;
  for (vector<FLAYER>::iterator it = veclayer.begin() + 1; it < veclayer.end();
       it++) {
    if (it->name == txt) {
      fl = *(it - 1);
      *(it - 1) = *it;
...
void CDraw::downlayer(const CString &txt) {
  FLAYER fl;
  for (vector<FLAYER>::iterator it = veclayer.begin(); it < veclayer.end() - 1;
       it++) {
    if (it->name == txt) {
      fl = *(it + 1);
      *(it + 1) = *it;
  ...
  for (int i = 0; i < fit; i++) {
    if ((fig[i]->transselectedstate()) &&
      ...
      if (im != 2) {
        ...
        while (bWorking) {
          bWorking = finder.FindNextFile();
          CFile file(finder.GetFilePath(), CFile::modeCreate |
          ...
          for (vector<FLAYER>::iterator iter = vveclayer.begin();
               iter < vveclayer.end(); iter++) {
            int i = iter->name.GetLength();
            file.Write(&i, sizeof(int));
            for (int j = 0; j < i; j++) {
              char c = iter->name[j];
...
void CDraw::surface(CString type, float vthick, bool vctop, bool vcbot,
                    CDC *hdc, int ext) {
  int l = 0;
  int pk = fit;
...
void CDraw::degel(CDC *hdc) {
  ...
  for (int i = 0; i < fit; i++) {
    if (!(fig[i]->getLayer()).clear) {
      if (fig[i]->transselectedstate()) {
        if ((fig[i]->getName() == CString("CBezier")) ||
            (fig[i]->getName() == CString("CCercle"))) {
          CBezier *ftp = new CBezier;
...
void CDraw::volume(CDC *hdc) {
  ...
  if (l > 1) {
    ...
    ftp->volume(hdc);
    /*fig=(CFigure**) realloc(fig,m+sizeof(CFigure*));*/ fig.resize(fig.size() +
                                                                    1);
    // m=_msize(fig);
    ftp->clear();
    ftp->setHandle(scount);
...
const int *CDraw::getVFint() { return &fit; }
...
void CDraw::setgeneralInterest(CSubject *s) { s->RegisterInterest(this); }
...
void CDraw::SendNotify(CString s, void *param[]) {
  if (s == "Light") {
    ...
    CBezier::M_spec.b = *spec;
    spec++;
    CBezier::m_gls = *spec;
    spec++;

    spec = (float *)param[1];
    CBezier::G_amb.r = *spec;
    spec++;
    CBezier::G_amb.g = *spec;
    spec++;
    CBezier::G_amb.b = *spec;
    spec++;

    spec = (float *)param[2];
    CBezier::S_diff.r = *spec;
    spec++;
    CBezier::S_diff.g = *spec;
    spec++;
    CBezier::S_diff.b = *spec;
    spec++;
...
void CDraw::getlightpara(void **param[]) {
  ...
  **spec = CBezier::S_spec.r * 255;
  spec++;
  **spec = CBezier::S_spec.g * 255;
  spec++;

  spec = (int **)param[1];
  **spec = CBezier::G_amb.r * 255;
  spec++;
  **spec = CBezier::G_amb.g * 255;
  spec++;
  **spec = CBezier::G_amb.b * 255;
  spec++;

  spec = (int **)param[2];
  **spec = CBezier::S_diff.r * 255;
  spec++;
  **spec = CBezier::S_diff.g * 255;
  spec++;
  **spec = CBezier::S_diff.b * 255;
  spec++;

  float **spec1 = (float **)param[3];
  **spec1 = CBezier::lsou.x;
  spec1++;
  **spec1 = CBezier::lsou.y;
  spec1++;
  **spec1 = CBezier::lsou.z;
  spec1++;
}

void CDraw::setcamInterest(CSubject *s) { setgeneralInterest(s); }
...
void CDraw::AddSpline2(vector<CVector> &v, vector<float> &fv, bool _fit,
                       CDC *hdc) {
  v_current = v.back();
  undoList.push("create");
  string scount;
...
void CDraw::AddLayer(string &layer_name, int layer_status, int layer_color,
                     string &layer_linetype, int layer_lineweight) {
  try {
...
void CDraw::AddTextStyle(string &style_name, string font_file,
                         float fixed_height, float width_factor) {
  FSTYLE g;
  g.handle++;
  g.name = style_name;
...
void CDraw::AddText(CDC *hdc, CVector &v, string &_text, float yheight,
                    string facename) {
  v_current = v;
  undoList.push("create");
  string scount;
  ...
  bool find = false;
  for (vector<FSTYLE>::iterator iter = vecstyle.begin(); iter < vecstyle.end();
       iter++) {
...
void CDraw::AddViewport(CDC *hdc, CVector &v0_wcs, CVector &v0_dcs,
                        CVector &vo_ucs, CVector &vox_ucs, CVector &voy_ucs,
                        float zoom_pc, float paper_width, float paper_height) {
  ...
  vt.setWorld(CVector(1, 0, 0, 0), CVector(0, 1, 0, 0), CVector(0, 0, 1, 0),
              v0_wcs);
...
CFigure *CDraw::getFigure(int i) { return fig[i]; }

short CDraw::getFigures() { return fig.size(); }
...
void CDraw::Close(CFigure *f, CDC *hdc) { f->closecurve(CVector(0, 0), hdc); }

void CDraw::Chamfer(CFigure *f1, float u1, CFigure *f2, float u2,
                    float length) {
  ...
  sf.Format(_T("%d"), count);
  scount = sf;
  if ((f1->getName() == pL.getName()) || (f1->getName() == pR.getName()) ||
      (f1->getName() == pQ.getName())) {
    vector<CVector> test;
  ...
  if (nbc == 0)
    return;
  if ((f2->getName() == pL.getName()) || (f2->getName() == pR.getName()) ||
      (f2->getName() == pQ.getName())) {
    vector<CVector> test;
  ...
  for (int i = 0; i < fit; i++) {
    ...
    if (fig[i]->getName() == pL.getName()) {
      ...
      *ftp = pL;
      ...
      delete fig[i];
      fig[i] = NULL;
    } else if (fig[i]->getName() == pR.getName()) {
      pR = *(CRectangle *)fig[i];
      pR.addvec(*arr, *(arr + 1));
      delete[] arr;
      CRectangle *ftp = new CRectangle;
      *ftp = pR;
      ...
      delete fig[i];
      fig[i] = NULL;
    } else if (fig[i]->getName() == image.getName()) {
      image = *(CImage *)fig[i];
      image.addvec(*arr, *(arr + 1));
      delete[] arr;
      CImage *ftp = new CImage;
      *ftp = image;
      ...
      delete fig[i];
      fig[i] = NULL;
    } else if (fig[i]->getName() == pB.getName()) {
      ...
      *ftp = textc;
      ...
      delete fig[i];
      fig[i] = NULL;
    } else if (fig[i]->getName() == lib->getName()) {
  ...
  if (f->getName() == pLine.getName()) {
    ...
    for (int j = 0; j < fit; j++) {
      ...
      if (fig[j]->inselectedstate()) {
        ...
        } else if (fig[j]->getName() == pRectangle.getName()) {
        ...
        } else if (fig[j]->getName() == pCircle.getName()) {
        ...
        } else if (fig[j]->getName() == pBezier->getName()) {
        ...
        } else if (fig[j]->getName() == pl.getName()) {
          pLib = (CLibS *)fig[j];
        ...
        } else if (fig[j]->getName() == textc.getName()) {
  ...
  if (f->getName() == pLine.getName()) {
    for (int j = 0; j < fit; j++) {
      ...
      if (fig[j]->inselectedstate()) {
        ...
        } else if (fig[j]->getName() == pC.getName()) {
  ...
  for (int i = 0; i < fit; i++) {
    ...
    //		if (fig[i]->inselectedstate())
    {
      ...
      if (vs[i]->getName() != pLibs.getName()) {
        ...
        if ((vs[i]->getName() == pLine.getName()) ||
            (vs[i]->getName() == pRectangle.getName())) {
          pLine = *(CLine *)vs[i];
          ...
          *ftp = pLine;
          ...
          fig[pk] = ftp;
          pk++;
        } else if (vs[i]->getName() == pBezier->getName()) {
...
void CDraw::setmouse(bool t) { mouseon = t; }
...
void CDraw::setcurrPoint(CVector v) { v_current = v; }
...
void CDraw::getdimL(int &length_type, int &length_name, int &precision) {
  if (CDimText::style == _T("linear-inch")) {
    length_type = 1;
    length_name = 1;
  } else if (CDimText::style == _T("linear-feet")) {
    length_type = 1;
    length_name = 2;
  } else if (CDimText::style == _T("linear-millimeter")) {
    length_type = 1;
    length_name = 3;
  } else if (CDimText::style == _T("linear-centimeter")) {
    length_type = 1;
    length_name = 4;
  } else if (CDimText::style == _T("linear-meter")) {
    length_type = 1;
    length_name = 5;
  }
...
void CDraw::fitToPage(bool _fit) { fittopage = _fit; }
...
bool CDraw::getSnapExten(CVector &v, CDC *hdc) {
  vector<CFigure *> fb, fe;
  for (int i = 0; i < fit; i++) {
    if (fig[i]->fsnap(v, v0, "nearest", hdc))
      return true;
  }
  return false;
}
...
bool CDraw::getSnapMidPoint(CVector &v, CVector &vl, CDC *hdc) {
  ...
  int t = fb.size();
  for (i = 0; i < t; i++) {
    if (fb[i]->fsnap(v, vl, "midpoint", hdc))
      return true;
  }
  return false;
}
...
bool CDraw::getSnapNearest(CVector &v, CVector &vl, CDC *hdc) {
  ...
  int t = fb.size();
  for (i = 0; i < t; i++) {
    if (fb[i]->fsnap(v, vl, "nearest", hdc))
      return true;
  }
  return false;
}
...
void CDraw::getsnap(vector<CProperty *> v) { snappy.setval(v); }
