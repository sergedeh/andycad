...
static const CLSID clsid = {0xe8559014,
                            0x9d11,
                            0x4c7b,
                            {0xb1, 0x7a, 0x1c, 0x96, 0x6e, 0x9a, 0x13, 0xda}};
...
BOOL CCadApp::InitInstance() {
  ...
  pDocTemplate =
      new CSingleDocTemplate(IDR_MAINFRAME, RUNTIME_CLASS(CCadDoc),
  ...
  // CG: This block was added by the MAPI component
  {
    ...
    if ((m_pMainWnd != NULL) && ((pMenu = m_pMainWnd->GetMenu()) != NULL) &&
        ((pFileMenu = pMenu->GetSubMenu(0)) != NULL)) {
      // Locate the "Sen&d..." menu item
      int i, nLength;
      for (i = 0, nLength = pFileMenu->GetMenuItemCount(); i < nLength; i++) {
        CString strSend;
      ...
      if (i == nLength) {
        ...
        pFileMenu->InsertMenu(nLength - 1, MF_BYPOSITION, ID_FILE_SEND_MAIL,
                              _T("Sen&d..."));
      }
    }
  }
...
BOOL CAboutDlg::PreTranslateMessage(MSG *pMsg) {
  ...
  return CDialog::PreTranslateMessage(
      pMsg); // CG: This was added by the ToolTips component.
}
...
LONG CCadModule::Unlock() noexcept {
  AfxOleUnlockApp();
  return 0;
}

LONG CCadModule::Lock() noexcept {
  AfxOleLockApp();
  return 1;
}
