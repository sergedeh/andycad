...
#include "..\MFCAndyCAD\MFCAndyCADView.h"
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(CMFCAndyCADView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(CMFCAndyCADDoc, (CObject*) m_pDocument->GetContainer());
