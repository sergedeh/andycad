...
HANDLE threadm[2000];
int threadc=0;
DWORD WINAPI TriDisplayp(LPVOID m)
{
	threadc++;
	t_param *mp=(t_param*) m;

	int pwidth=mp->pwidth;
	int pheight=mp->pheight;
	int i=mp->i;
	CTriangle *t=mp->o;
//	t->Draw_Triangle_2D(hd,mp->mytext,pwidth,pheight,mp->l);//display(hdc,loc,lit);
...
/*
DWORD WINAPI TrilistDisplayp(LPVOID m)
{
	threadc++;
	t_param *mp=(t_param*) m;
...
/*
	...
	CDC *cd=CDC::FromHandle(hd);
	if(hwnd==NULL)
		hwnd=((CCadView*)cd->GetWindow())->GetSafeHwnd();

	CRect wcr;
	view->GetClientRect(&wcr);
	cd->DPtoLP(&wcr);
	
	CRect R(CPoint(0,0),CSize(CSize(view->GetTotalSize().cx,-view->GetTotalSize().cy)));//=pDoc->m_objects->getgrid(CSize(GetTotalSize().cx,-GetTotalSize().cy));		
	...
	CSize sz(bn.Size().cx,bn.Size().cy);
	cd->LPtoDP(&sz);

	HBITMAP bit=CreateCompatibleBitmap(hdc,sz.cx,sz.cy);
	HDC hmem=CreateCompatibleDC(hdc);

	
	int uj=GetMapMode(hdc);
	SetMapMode(hmem,uj);
	CSize s;
	GetWindowExtEx(hdc,&s);
	SetWindowExtEx(hmem,s.cx,s.cy,NULL);
	GetViewportExtEx(hdc,&s);
	SetViewportExtEx(hmem,s.cx,s.cy,NULL);
	CPoint pl;
	GetWindowOrgEx(hdc,&pl);
	SetWindowOrgEx(hmem,pl.x,pl.y,NULL);

	BITMAP bitmap;
	int j=GetObject(bit,NULL,NULL);
	GetObject(bit,j,&bitmap);
	BITMAPINFO bitinfo;

	bitinfo.bmiHeader.biSize=sizeof(BITMAPINFOHEADER);
	bitinfo.bmiHeader.biWidth=bitmap.bmWidth;
	bitinfo.bmiHeader.biHeight=bitmap.bmHeight;
	bitinfo.bmiHeader.biPlanes = 1;
	bitinfo.bmiHeader.biBitCount = 32;
	bitinfo.bmiHeader.biCompression = BI_RGB;

	GetDIBits(hdc,bit,0,bitmap.bmHeight,NULL,&bitinfo,DIB_RGB_COLORS);
//	if(bits==NULL)
	BYTE* bits=new BYTE[bitinfo.bmiHeader.biSizeImage*4];
	GetDIBits(hdc,bit,0,-bitmap.bmHeight,(LPVOID)bits,&bitinfo,DIB_RGB_COLORS);


	int p1height=bitmap.bmHeight;
	int p1width=bitmap.bmWidth;
*/
/*
	for(vector<CTriangle*>::iterator iter=me->trilist.begin();iter!=me->trilist.end();iter++)
	{
//		if((iter->vertex1->isOn())||(iter->vertex2->isOn())||(iter->vertex3->isOn()))
//		{

/*			CVector v11=iter->vertex1->v;
			v11.viewi();
			CVector v22=iter->vertex2->v;
			v22.viewi();
			CVector v33=iter->vertex3->v;
			v33.viewi();
			CVector v1(v11,v22);
			CVector v2(v22,v33);
			if((v1*v2).dot(v1.getcamor())>0)
			{*/
/*	(*iter)->isDegenerate(hdc);
	(*iter)->Draw_Triangle_2D(hdc,bits,nb.bmHeight,nb.bmWidth,mp->mytext,pwidth,pheight,mp->l);//display(hdc,loc,lit);

	}
	bmp->SetBitmapBits(d,bits);

/*	SetDIBits(hmem,bit,0,bitmap.bmHeight,(LPVOID)bits,&bitinfo,DIB_RGB_COLORS);
	bit=(HBITMAP)SelectObject(hmem,bit);
*/
//	CRect p(0,0,wr.Size().cx,-wcr.Size().cy);
	
//		SelectObject(hmem,GetStockObject(LTGRAY_BRUSH));
//		Rectangle(hmem,0,0,bn.Size().cx/2,-bn.Size().cy/2);

//	j=SetMapMode(hdc,MM_TEXT);
//	SetMapMode(hmem,MM_TEXT);

//	BitBlt(hdc, 0,0,bn.Size().cx,-bn.Size().cy,hmem,0,0,SRCCOPY);
/*	delete []bits;
//	SetMapMode(hdc,j);


/*	for(iter=me->trilist.begin();iter!=me->trilist.end();iter++)
	{
//		if((iter->vertex1->isOn())||(iter->vertex2->isOn())||(iter->vertex3->isOn()))
//		{
...
/*			CVector v11=iter->vertex1->v;
			...
			if((v1*v2).dot(v1.getcamor())>0)
			{*/
/*	iter->Draw_Triangle_2D(hdc,mp->mytext,pwidth,pheight,mp->l);//display(hdc,loc,lit);

	}
*/
/*	delete m;
//	CloseHandle(threadm[i]);
	threadc--;
return NULL;
}*/
//////////////////////////////////////////////////////////////////////
...
CMesh::~CMesh()
{
	...
	for(iter=trilist.begin();iter!=trilist.end();iter++)
	{
		delete *iter;
	}
	trilist.clear();
	...
	vertexlist.clear();
	for(iter2=svertexlist.begin();iter2!=svertexlist.end();iter2++)
	{
		delete iter2->second;
	}
	svertexlist.clear();
...
//	iter->Draw_Triangle_2D(hd,bits,nb.bmHeight,nb.bmWidth,mytext,pwidth,pheight,l);//display(hdc,loc,lit);
...
/*			CVector v11=iter->vertex1->v;
			...
			if((v1*v2).dot(v1.getcamor())>0)
			{*//*
			t_param* t=new t_param;
	//			iter->Draw_Triangle_2D(hdc,mytext,pwidth,pheight,l);//display(hdc,loc,lit);
		 // 	t->hwnd=(hdc->GetWindow())->GetSafeHwnd();//m_hDC;
			t->mytext=mytext;
			t->pwidth=pwidth;
			t->pheight=pheight;
			t->i=threadc;
			...
			HANDLE m/*threadm[threadc]*//*=CreateThread(NULL,0,TrilistDisplayp,(LPVOID)t,0,NULL);
			WaitForSingleObject(m,INFINITE);
		   i++;w++;
/*		   if(i==50)
		   {
			WaitForMultipleObjects(i,threadm,TRUE,INFINITE);
	     // We don't need the thread handles anymore.  Close'em!
 			for (int j= 0; j <i ; j++ )
            CloseHandle( threadm[j] );
			i=0;
		   }*/
...
//	}
//  delete t;
/*	for(int j=0;j<i;j=j+50)
	{
		WaitForMultipleObjects(50,&threadm[j],TRUE,INFINITE);
	     // We don't need the thread handles anymore.  Close'em!
 			for (int p= 0; p <50 ; p++ )
            CloseHandle( threadm[p] );
	}
*//*	int p=i;
	i=i%50;
	WaitForMultipleObjects(i,&threadm[p],TRUE,INFINITE);
	// We don't need the thread handles anymore.  Close'em!
 	for (int j= 0; j <i ; j++ )
       CloseHandle( threadm[j] );
	DWORD ptick=GetTickCount()-tick;
//	CString s;
//	s.Format("time spend to render:%d",ptick);
//	AfxMessageBox(s);
*/
}
...
/*	itri tr;CTriangle trc;
	for(tr=trilist.end()-1;tr>tbegin();tr--)
	{
		if(tr->getcent().getDepth()>(tr-1)->getcent().getDepth())
		{
			trc=*(tr-1);
			*(tr-1)=*tr;
			*tr=trc;
...
/*	for(vector<CEdge>::iterator iter1=edgelist.begin();iter1!=edgelist.end();iter1++)
	{
		iter1->savefile(file);
	}*/
