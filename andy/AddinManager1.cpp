...
BOOL CAddinManager::LoadAllAddins()
{
	...
	if(SUCCEEDED(hResult))
	{
		...
		hResult = m_ProjectFrameworkObject.CoCreateInstance(CLSID_AndyCad);
	}
	if(!SUCCEEDED(hResult))
	{
		AfxMessageBox("AndyCad Addin Object Failed");
		return FALSE;
	}
/*	AfxConnectionAdvise((LPUNKNOWN)m_ProjectFrameworkObject,
														  DIID__IAndyCadEvents,
...
BOOL CAddinManager::InvokeAddinMenuItem(UINT nID)
{
	try
	{
		...
		{
			CString strFunction="call "+AddinCommadInfo.m_strFunction;
			//Get the DispId so call the invoke function after filling the parameters
			CMainFrame* mg=(CMainFrame*)::AfxGetMainWnd();
			mg->RunMacro(strFunction);
...
HRESULT CAddinManager::LoadAllAdins(BSTR strAddinLoadingInfo,IAndyCad* pProjectFramework)
{
	try
	{
		...
		{
			//Load All addins
			CATID catid = CATID_ANDYCAD_ADDINS;
			CLSID clsid[40];
			...
			do
			{
				...
				CCadApp::m_AddinManager.SetAddinCount(iAddinCount); 
				for (long i = 0; i < (long)num; i++)
				{
					IAndyCadAddin *pProjectFrameworkAddin = NULL;
					
					hr = CoCreateInstance(clsid[i], NULL,CLSCTX_INPROC_SERVER,IID_IAndyCadAddin, (void **)&pProjectFrameworkAddin);
			...
			CMacroManager macro;
			macro.LoadAllMacros(j);
			iAddinCount=j;
			CCadApp::m_AddinManager.SetAddinCount(iAddinCount); 


			if(!iAddinCount) return E_FAIL;
...
void CAddinManager::SendStatus(CString s)
{
m_ProjectFrameworkObject->SendStatus(s.AllocSysString());
}
