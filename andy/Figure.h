...
public:
	...
	bool edgeperpendicularconstraint(pair<CFigure*,int> f1,pair<CFigure*,int> f2);
	bool edgeverticalconstraint(pair<CFigure*,int> f1);
	bool edgehorizontalconstraint(pair<CFigure*,int> f1);
	bool edgeconstraint(pair<CFigure*,int> f1,pair<CFigure*,int> f2);
	bool pointconstraint(pair<CFigure*,int> f1,pair<CFigure*,int> f2);
	void cleancurve();
	...
	void intersect(CFigure* f, CVector& v);
	bool inBox(CVector v);
	void Extend(CFigure* f);
	void offset(CFigure* f, float u);
	...
	virtual CMesh* Clip(CFigure* f,bool side=false);
	virtual CMesh* Clip0(CFigure* f);
	...
	CFigure* Offset(float u);
	virtual float Min_dist_toEdge(CVector v);
	virtual void OffsetMesh(float u, CFigure* f);
	CFigure* copybyL(CFigure* line);
	...
	virtual void connectTo(CFigure *f, float u1,float u2, int al=0);
	virtual void connectTo(CVector p, float u1,bool ref=false);
//	virtual void alignWithC(CFigure *f, bool withx=true,bool withy=true,bool withz=true);
	virtual void alignWith(CFigure *f, float u=-1);
...
//	void selectMP(CVector point);
	...
	virtual void degel();
	virtual void copy(const CFigure* fig);
	virtual void add(const CFigure *fig);
	virtual void surface(CDC *hdc);
	CString getInfName();
	void setinfo(FINFO f);
	virtual bool fsnap(CVector& v,CVector &vl,LPCTSTR snapmode,CDC* hdc);
	...
	vector<CFigure*> translatepArray(bool of,int nbrow,int nbcol, double distx,double disty,int level,double distl);
	vector<CFigure*> RotateCopyP(CDC *hdc,double ang,int nnb,int nblevel,int dlevel, bool align);
	...
	virtual CPoint setfont(CDC *hdc,LOGFONT font,COLORREF ccl,CHARFORMAT cf);
	...
	CFigure* copybyC(CDC *hdc,CVector center);
	...
	virtual vector<CFigure*> breakcurve2(CVector v1,CVector v2);
