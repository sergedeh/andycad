...
CTextCad::CTextCad(fstream &file,CDC* hdc)
{
	rec.setdistprec(5);
	pfont=new CFont;
...
CGPoint CTextCad::updatecurs(int i)
{
	CRect ro(box.corner(0),box.corner(3));
	CVector vr=ro.BottomRight();
	CVector vl=ro.TopLeft();
...
CGPoint CTextCad::drawtext(CDC *hdc, int nchar)
{
/*	CRect ro(box.corner(0),box.corner(3));
	CVector vr=ro.BottomRight();
	CVector vl=ro.TopLeft();
...
CGPoint CTextCad::deleteback(CDC *hdc)
{
	if(sbiter!=seiter)
    {
		int k=seiter;
 ...
 */
CGPoint CTextCad::entertext(CDC *hdc)
{
	return drawtext(hdc,'\n');
    /*	CRect ro(box.corner(0),box.corner(3));
     CVector vr=ro.BottomRight();
...
CGPoint CTextCad::kbmovecurs(CDC *hdc,string& d)
{
...
void drawAsMetafile()
{
    /*
     ...
     stFR.rcPage=stFR.rc=CRect(CGPoint(0,0),sz1);
     stFR.chrg.cpMin=0;
     stFR.chrg.cpMax=-1;
     hdc->LPtoDP(rt2);
     hdc->BeginPath();
...
void CTextCad::setpara(string l)
{
...
/*	edge_on=true;
	...
	CFontDialog dlg;
	CDC* rhdc;
	CGPoint p1=CGPoint(mesh.Vertex(1)->v)-CGPoint(mesh.Vertex(0)->v);
	...
	if(rich->m_hWnd==NULL){
		...
		CGPoint pyheight=CVector(cf.yHeight,0);
		CGPoint pyheight1=CVector(0,0);
	...
	if(!rich->IsWindowVisible())
	{
		...
		hdc->LPtoDP(&pol);
		CGPoint po=mesh.Vertex(0)->v;//rt2->TopLeft()
		hdc->LPtoDP(&po);
	...
	for(itmap i=ltext.begin();i!=ltext.end();i++)
    {
        ...
        else
        {
            ...
            i->second.v=j->second.v;
            i->second.v.x=j->second.v.x+fmap[ltext[j->first].f].abc[ltext[j->first].c].abcfA+fmap[ltext[j->first].f].abc[ltext[j->first].c].abcfB+fmap[ltext[j->first].f].abc[ltext[j->first].c].abcfC;
            if(i->second.v.x>CGPoint(vr).x)
            {
                i->second.v.x=CGPoint(vl).x;
...
CGPoint CTextCad::setfont(CDC *hdc,LOGFONT font,COLORREF ccl,CHARFORMAT cf)
{
/*	if(rich->m_hWnd!=NULL){
		long u=-1,v=-1;
		rich->GetSel(u,v);
...
CGPoint CTextCad::deletetext(CDC *hdc)
{
...
CGPoint CTextCad::textend(CDC *hdc)
{
	vectext.pop_back();
	vecpos.pop_back();
...
void CTextCad::savefiledxf(CStdioFile& file)
{
	...
	if(n<255)
	{
		...
		file.WriteString(cf.szFaceName);file.WriteString("\n");
...
*///****************
void CTextCad::savefile(fstream &file)
{
/*	CLine::savefile(file);
 	long length=rich->GetTextLength();
...
void CTextCad::openfile(fstream &file,CDC* hdc)
{
	CLine::openfile(file);
	draw3D(hdc);
 	long length;
...
void CTextCad::assign(CDC *hdc,CVector p,string s,CHARFORMAT* _cf)
{
 	mtext=s;
	CRect rt;
	rt.SetRectEmpty();
...
/*	if(_cf==NULL)
    ...
    //	rt.InvertRect();
	CGPoint p1=rt.TopLeft();
	CGPoint p3=rt.BottomRight();;
	CGPoint p2=CGPoint(p3.x,p1.y);
	CGPoint p4=CGPoint(p1.x,p3.y);
	CVector v1=p1;
	CVector v2=p2;
	CVector v3=p3;
	CVector v4=p4;
	...
	rec.copy(&rc);
	getCharge();
	p1=CGPoint(mesh.Vertex(1)->v)-CGPoint(mesh.Vertex(0)->v);
...
void CTextCad::setText(string vtext,CDC* hdc)
{
/*	mtext=vtext;
	updatecore();
	CVector vr=box.corner(3);//rop.BottomRight();
