...
/*		CPen pen;
        ...
        if(select==pnt)
        {
            ...
            for (piterb=ptbegin.begin();piterb<ptbegin.end();piterb++)
            {
                ...
                hdc->MoveTo(ptsBegin);
                //					hdc->Ellipse(CRect(CGPointv(pb.x-13, pb.y+13),CGPointv(pb.x+13, pb.y-13)));
            }
		...
		if(!core.empty())
		{
			if(select==full)
			{
				for (piterb=core.begin();piterb<core.end()-3;piterb++)
				{
					...
					if(*piterb!=core[8])
					{
						hdc->Ellipse(CRect(CGPointv(pbonbon.x-13, pbonbon.y+13),CGPointv(pbonbon.x+13, pbonbon.y-13)));
					}
				}
				...
				hdc->Ellipse(CRect(CGPointv(pt0.x-13, pt0.y+13),CGPointv(pt0.x+13, pt0.y-13)));
				...
				hdc->Ellipse(CRect(CGPointv(pt1.x-17, pt1.y+17),CGPointv(pt1.x+17, pt1.y-17)));
        ...
        for(vector<CEdge*>::iterator it=vel.begin();it!=vel.end();it++)
        {
            ...
            if(r.top!=r.bottom)
            {
                ...
                if(mode_texture==wrap)
                {
                    ...
                    s.du=0;//-(s.u_top-s.u_bottom)/pheight;
...
void CFigure::Hatch(CDC *hdc)
{
	...
	try{
        ...
        CGPoint p1=CGPointv(ptend[1])-CGPointv(ptend[0]);
        ...
        if(v.size()!=0)
        {
            ...
            gi->r.bottom=ybottom;
            _h=ytop-ybottom;
            //	vi=min_element(v.begin(),v.end(),pointer_to_binary_function<CGPoint,CGPoint,bool>(isxleft));
            //	double xleft=vi->x;
...
void CFigure::setcore()
{
	...
	CGPoint cr=rotateCenter;
	cr.x+=100;
 	core[13]=cr;//rotateCenter + CGPointv(100,0);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
/*void CFigure::savefiledxf(CStdio& file)
{
    
}*///************************************
void CFigure::savefile(fstream& file)
{
...
void CFigure::savefile(CDC *hdc,fstream& file)
{
    
}

void CFigure::openfile(CDC *hdc,fstream& file)
{
    
}


void CFigure::openfile(fstream& file)
{
	init();
...
long CFigure::Readfile(fstream& file)
{
    init();
...
/*	if (prevrotateCenter==v)
        ...
        //	if (absc(CVector(ro,vo))<25)//((rotateCenter-25).operator <(prevpoint))&&(prevpoint.operator <(rotateCenter+25)))
	{
        ...
        int old=hdc->SetROP2(R2_NOTXORPEN);
        CGPoint pr(prevrotateCenter);
        CGPoint r(rotateCenter);
        
        {
            CGPoint pt0=pr;//(rotateCenter);
            hdc->MoveTo(pt0);
            //		hdc->TextOut(pt0.x,pt0.y,"Ol");
            CGPoint pt1(core[14]);
            CPen pen3;
...
vector<CFigure>* CFigure::RotateCopyP(CDC *hdc,CVector point,vector<CFigure>* pbulk, bool align)
{
	...
	double diff=(argu(piv)<argu(point))?2*pi2-(argu(piv)-argu(point)):2*pi2+(argu(piv)-argu(point));
...
void CFigure::setType(string type)
{
...
CGPoint CFigure::drawtext(CDC *hdc, int nchar)
{
	CGPoint p= CGPointv(0,0);
	return p;
    
}

CGPoint CFigure::deleteback(CDC *hdc)
{
	CGPoint p= CGPointv(0,0);
	return p;
...
CGPoint CFigure::texthome(CDC *hdc)
{
	CGPoint p= CGPointv(0,0);
	return p;
...
CGPoint CFigure::deletetext(CDC *hdc)
{
	CGPoint p= CGPointv(0,0);
	return p;
...
CGPoint CFigure::textend(CDC *hdc)
{
	CGPoint p= CGPointv(0,0);
	return p;
...
CGPoint CFigure::entertext(CDC *hdc)
{
	CGPoint p= CGPointv(0,0);
	return p;
...
CGPoint CFigure::kbmovecurs(CDC *hdc, string& d)
{
	CGPoint p= CGPointv(0,0);
	return p;
...
/*
CGPoint CFigure::setfont(CDC *hdc,LOGFONT font,COLORREF ccl,CHARFORMAT cf)
{
	CGPoint p;
	return p;
...
bool CFigure::fsnap(CVector &v,CVector &vl, string snapmode,CDC *hdc)
{
	string snm=snapmode;
	CVector h;
	bool hb=false;
...
void CFigure::SendNotify(string s, void *param[])
{
	if(s=="Transparency")
	{
...
void CFigure::setTexture(string s,CDC* hdc)
{
    cl=true;
    texture_file=s;
...
{
    CVector no=box.corner(0);
    CVector n2=box.corner(2);
	CRay r1(no,n2);
