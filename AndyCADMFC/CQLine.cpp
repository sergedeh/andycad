...
void CQLine::polystate(CVector pos)
{
	...
	switch (lstate)
	{
        ...
        case(L2):
		{
			...
			if(abs((long)vkp.x)>abs((long)vkp.y))
			{
				float f=abs((long)vk.dot(vrx));
				if(f!=0)
                    ptsEnd=ptsBegin+vrx*absc(vp)*vk.dot(vrx)/f;
			}
			else
			{
				float f=abs((long)vk.dot(vry));
				if(f!=0)
                    ptsEnd=ptsBegin+vry*absc(vp)*vk.dot(vry)/abs((long)vk.dot(vry));
			}
...
void CQLine::savefile(fstream &file)
{
	CLine::savefile(file);
}


void CQLine::openfile(fstream &file)
{
	CLine::openfile(file);
	name="CQLine";
}
long CQLine::Readfile(fstream &file)
{
	return CLine::Readfile(file);
}
CQLine::CQLine(fstream &file)
{
    cancharge=true;
    setReflineP(v0,v0);
    openfile(file);
