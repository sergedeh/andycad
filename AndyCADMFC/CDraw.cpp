...
bool CDraw::add(string figure,bool fPrevLine,CDC* hdc,CVector point)
{
	if(fit>0)
		curr_fig=fig[fit-1];
...
void CDraw::Create(CDC *hdc,string figure,CVector point)
{
	if(snap)
	{
...
void CDraw::finaladd( CDC *hdc,string figure, CVector pos2,string option)
{
...
*///*****************
void CDraw::save(CDC *hdc,fstream& file)
{
/*    //	if(!candraw) return;
...
void CDraw::open(CDC *hdc,fstream& file)
{
 /*
	candraw=false;
    //	try{
...
void CDraw::save00(CDC *hdc,fstream& file)
{
    //	if(!candraw) return;
/*    vector<INDEX> index(fit);
    ///////////////////////////////////////////////////////////
...
void CDraw::open00(CDC *hdc,fstream& file)
{
 /*
	try{
        candraw=false;
...
void CDraw::import(CDC *hdc,fstream& file)
{
...
void CDraw::undo(string& cmd,bool rdo)
{
	...
	if(undoList.size()>0)
	{
		...
		if (obj=="delete")
		{
            ...
            CFigure* f=m->getF();
            vector<CMemento*>::iterator _pi;
            for(_pi=vmemento.begin();_pi!=vmemento.end();_pi++)
            {
                if((*_pi)->getF()==f)
                    (*_pi)->setF(fig[fit]);
            }
            for(_pi=rmemento.begin();_pi!=rmemento.end();_pi++)
            {
                if((*_pi)->getF()==f)
                    (*_pi)->setF(fig[fit]);
            }
...
CRect CDraw::grid(CDC *rhdc,CDC *hdc,CSize cp)
{
    ...
    CGContextSetRGBFillColor(hdc->context, 255, 255, 255, 1);// 3
    
    
	CVector v0(k.getcamor()),vx(100,0,0,0),vy(0,100,0,0),v1,vz(0,0,100,0);
	CRect p(0,0,cp.cx,cp.cy);
    
    CGContextFillRect (hdc->context, CGRectMake (p.top, p.left, p.bottom, p.right ));// 4
	snappy.displaygrid(hdc,this);
	CVector t0(v1.getcamor()),tx(v1.getcamx()),ty(v1.getcamy()),tz(v1.getcamz());
    
    CGContextSetRGBFillColor(hdc->context, 0, 0, 0, 1);// 3
    CGContextSetTextDrawingMode(hdc->context, kCGTextFillStroke);
    CGContextSelectFont(hdc->context, "Helvetica-Bold", 1, kCGEncodingMacRoman);
    CGContextSetFontSize(hdc->context, 1.);

    CGContextMoveToPoint(hdc->context,v0.x,v0.y);
    CGContextShowTextAtPoint(hdc->context,v0.x,v0.y,"O",1);
	vx=v0+vx;
    CGContextAddLineToPoint(hdc->context,vx.x,vx.y);
    CGContextShowTextAtPoint(hdc->context,vx.x,vx.y,"X",1);
    CGContextMoveToPoint(hdc->context,v0.x,v0.y);
	vy=vy+v0;
    CGContextAddLineToPoint(hdc->context,vy.x,vy.y);
    CGContextShowTextAtPoint(hdc->context,vy.x,vy.y,"Y",1);
    vz=v0+vz;
    CGContextMoveToPoint(hdc->context,v0.x,v0.y);
    CGContextAddLineToPoint(hdc->context,vz.x,vz.y);
    CGContextShowTextAtPoint(hdc->context,vz.x,vz.y,"Z",1);
	display(rhdc,hdc);
...
CGPoint CDraw::drawtext(CDC *hdc, int nchar)
{
	CTextCad textc;
	for(int i=0;i<fit;i++)
	{
...
CGPoint CDraw::deletebk(CDC *hdc)
{
	CTextCad test;
	for(int i=0;i<fit;i++)
	{
...
CGPoint CDraw::setFont(CDC *hdc,LOGFONT font,COLORREF ccl,CHARFORMAT cf)
{
	CTextCad textc;
	int j1=vs.size();
	bool c=false;
...
void CDraw::savePNG(CDC *hdc,CBitmap* bitmap, fstream &file)
{
	CImage img;
...
void CDraw::InitlibDxf(CLibdialog& lbtctl,fstream& file)
{
...
void CDraw::InitlibAff(CLibdialog& lbtctl,fstream& file)
{
/*
	_tctl=(CTreeCtrl*)_lbtctl->GetDlgItem(IDC_LIBT1);
	int tversion;
...
void CDraw::Insertlib(CDC *hdc,string &f, string &l,CVector pos)
{
/*	CLibS *pLibs;
	CInter pInter;
	CFigure* fg;
...
void CDraw::addlayer(CListCtrl &tcl,string ltext)
{
/*	LVITEM it;
	it.mask=LVIF_TEXT|LVIF_IMAGE|\
    LVIF_STATE;
...
void CDraw::removelayer(string &txt)
{
	for(vector<FLAYER>::iterator it=veclayer.begin();it<veclayer.end();it++)
	{
		if(it->name==txt)
...
void CDraw::downlayer(string &txt)
{
	FLAYER fl;
...
void CDraw::setclayer(string txt,int state)
{
	for(vector<FLAYER>::iterator it=veclayer.begin();it<veclayer.end();it++)
	{
		if(it->name==txt)
...
void CDraw::setcurlayer(CListCtrl& tcl,string txt)
{
	for(vector<FLAYER>::iterator it=veclayer.begin();it<veclayer.end();it++)
	{
		if(it->name==txt)
...
void CDraw::lockcurlayer(CListCtrl &tcl, string txt)
{
...
void CDraw::unlockcurlayer(CListCtrl &tcl, string txt)
{
	for(vector<FLAYER>::iterator it=veclayer.begin();it<veclayer.end();it++)
    {
        if(it->name==txt)
...
void CDraw::setobjinfo(string aut, string desc, string name, double cost)
{
    //	for(int i=0;i<fit;i++)
    //	{
    //		if(fig[i]->transselectedstate())
...
void CDraw::surface(string type,float vthick,bool vctop,bool vcbot,CDC *hdc,int ext)
{
	int l=0;
	int pk=fit;
	...
	int j=vs.size();
	CLine pLine1;
	string line_name=pLine1.getName();
	string rectangle_name=pRectangle.getName();
...
void CDraw::degel(CDC *hdc)
{
	...
	for(int i=0;i<fit;i++)
	{
		if(!(fig[i]->getLayer()).clear)
		{
			if(fig[i]->transselectedstate())
			{
				if((fig[i]->getName()==string("CBezier"))||(fig[i]->getName()==string("CCercle")))
				{
					CBezier *ftp=new CBezier;
...
void CDraw::perpective(string s)
{
    CCAM v;
    CAABB box;
    for (int i=0;i<fit;i++)
...
void CDraw::fill(CDC *hdc, CGPoint pos)
{
	bool b=false;
	CTextCad textc;
	for (int i=0;i<fit;i++)
...
void CDraw::setTexture(string s,CDC* hdc)
{
	CTextCad textc;
	int g=vs.size();
...
void CDraw::setsnapplane(string plane)
{
	CTVector v;
	v.setsnapplane(plane);
}

void CDraw::setFFD(string option)
{
	CTextCad textc;
    //	CBezier bz;
    //	CCercle cl;
...
void CDraw::SendNotify(string s, void *param[])
{
	if(s=="Light")
	{
...
void CDraw::surface2(string type,float vthick,short cap,int ext)
{
	bool vctop=cap&&1;
	bool vcbot=cap&&2;
	int l=0;
...
void CDraw::setText(string text)
{
    /*	CTextCad textc;
     for(int i=0;i<fit;i++)
     {
...
void CDraw::setPage(CSize p)
{
	...
	CGPoint p1=CGPointv(p0.x+p.cx,p0.y+p.cy);
...
/*	CPen pen(PS_COSMETIC,0.1,RGB(192,192,192));
	...
	tv.VpopTransform();
	tv.popTransform();
	CRect scr(CGPointv(0,0),CSize(CGPointv(pp)));
	scr.NormalizeRect();
	CVector v;
...
void CDraw::drawmouse(CDC *hdc,CGPoint mouse,bool in)
{
/*	CVector vs=cam.getScreen();
...
void CDraw::getPoint(CGPoint p,float& u1,float& u2,float& u3,CDC *hdc)
{
    CVector v(p);
...
{
	if(CDimText::style=="linear-inch")
	{
		length_type=1;
		length_name=1;
	}else
        if(CDimText::style=="linear-feet")
        {
            length_type=1;
            length_name=2;
        }else
            if(CDimText::style=="linear-millimeter")
            {
                length_type=1;
                length_name=3;
            }else
                if(CDimText::style=="linear-centimeter")
                {
                    length_type=1;
                    length_name=4;
                }else
                    if(CDimText::style=="linear-meter")
                    {
                        length_type=1;
                        length_name=5;
                    }
...
void CDraw::setdimL(int length_type,int length_name,int precision)
{
	...
	{
		if(length_name==1)
			CDimText::style="linear-inch";
		else
            if(length_name==2)
                CDimText::style="linear-feet";
            else
                if(length_name==3)
                    CDimText::style="linear-millimeter";
                else
                    if(length_name==4)
                        CDimText::style="linear-centimeter";
                    else
                        if(length_name==5)
                            CDimText::style="linear-meter";
	}
	else
        if(length_type==2)
        {
            CDimText::style="linear-architect";
        }else
            if(length_type==3)
            {
                CDimText::style="linear-feet";
...
{
	if(length_type==2)
		CDimText::astyle="angle-degree";
	else
		CDimText::astyle="angle-radian";
...
{
	printpage.updatecore();
    // CGPoint p00=printpage.ptbegin[0];
    // printpage.TranslateTo(getcamor());
	CGPoint p01=printpage.box.corner(1);
	CGPoint p00=printpage.box.corner(0);
	CAABB box2;
	int j=0;
	for(vector<FLAYER>::iterator it=veclayer.begin();it<veclayer.end();it++)
	{
	...
	CGPoint ap01=box2.corner(1);
	CGPoint ap00=box2.corner(0);
	float u1=sqrt(pow(p01.x-p00.x,2)+pow(p01.y-p00.y,2));
	float u2=sqrt(pow(ap01.x-ap00.x,2)+pow(ap01.y-ap00.y,2));
	u1=u2/u1;
    //	CGPoint p01=printpage.box.corner(1);
	CGPoint p02=printpage.box.corner(2);
    //	CGPoint ap01=box.corner(1);
	CGPoint ap02=box2.corner(2);
