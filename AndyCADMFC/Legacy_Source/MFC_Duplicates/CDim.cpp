...
CDim::CDim(fstream &file,CDC* hdc)
{
	state=false;
	openfile(file,hdc);
	name="CDim";
...
void CDim::buildDim(CDC* hdc,float u, float v)
{
	...
	if((style==horiz)||(style==vert)||(style==free))
	{
        ...
        getEl(1)->computedist();
        ((CDimText*)getEl(5))->initText("linear-meter",hdc,pr,getEl(1)->myinfo.distance);
...
void CDim::SendNotify(string s, void *param[])
{
	if(s=="move")
	{
...
void CDim::savefile(fstream &file)
{
//	file.Write(&style,sizeof(STYLE));*********
	CLibS::savefile(file);
}

void CDim::openfile(fstream &file,CDC* hdc)
{
...
long CDim::ReadFike(fstream &file)
{
