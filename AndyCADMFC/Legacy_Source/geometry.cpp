...
// The point calculated may lie outside [start,end]
{
	if (ABS(start.x-end.x) < EPSILON)				// start-end form a vertical line
		return CVector(start.x, P.y, start.z);
	
	if (ABS(end.y - start.y) < EPSILON)				// start-end form a horizontal line
		return CVector(P.x, start.y, start.z);
	...
	return CVector(X, X*Slope + Intercept, start.z);
}

REAL Angle(CVector p1, CVector p2, CVector p3)
...
REAL TriArea2(CVector p1, CVector p2, CVector p3)
// Calculates twice the area of the 3D triangle given by p1,p2,p3, Positive if CCW
{
...
BOOL IntersectProp(CVector a, CVector b, CVector c, CVector d)
...
BOOL Between(CVector a, CVector b, CVector c)
...
BOOL Intersect(CVector a, CVector b, CVector c, CVector d)
...
CPolygon::CPolygon(vector<CVector>& b)
{
	m_point=b;
	m_n=b.size();
}
...
BOOL CPolygon::InsertAt(int nPosition, CVector P)
{
	if (!SetSize(GetSize()+1)) return FALSE;
...
BOOL CPolygon::GetAttributes(REAL *pArea, CVector *pCentroid, CVector *pNorm,
...
// Calculates and returns the polygons Area, centroid, norm, slope and aspect.
{
	...
	*pNorm     = CVector( D2Real(0.0), D2Real(0.0), D2Real(0.0) );
	...
	*pCentroid = *pCentroid/3.0;
	pNorm->Normalise();

	if (*pArea > EPSILON) 		// Triangle or greater
		*pCentroid =*pCentroid/(*pArea);
...
CVector CPolygon::Centroid()
{
	static REAL TotalArea;
	static CVector Centre;

	Centre = CVector( D2Real(0.0), D2Real(0.0), D2Real(0.0) );
	TotalArea = D2Real(0.0);

	for (int i=0; i < GetSize()-2; i++) {
		CVector MidPoint = (Point(0) + Point(i+1) + Point(i+2)) / 3.0;
...
// Given a CVector, returns the nearest CVector on the given polygon, with the option
// of also returning the index into the CPolygon point array of that point.
// Proximity is based on 2D, not 3D calculations.
CVector CPolygon::GetClosestPoint(CVector p, int *nIndex/*=NULL*/)
{
...
BOOL CPolygon::RInCone(int i, int j)
// 	Returns true iff the diagonal (i,j) is striclty internal to the polygon in 
//  the neighborhood of the i endpoint.
{
	int	iPlus1  = (i + 1)       % GetSize();
	...
	/* If P[i] is a convex vertex [ i+1 left or on (i-1,i) ]. */
	if ( RightOn(m_point[iMinus1], m_point[i], m_point[iPlus1]) )
		return	!Right( m_point[i], m_point[j], m_point[iMinus1] )
		     &&	!Right( m_point[j], m_point[i], m_point[iPlus1]  );

	/* Assume (i-1,i,i+1) not collinear. */
	/* else v_i is reflex. */
	else
		return	!(	RightOn( m_point[i], m_point[j], m_point[iPlus1]  )
		            &&	!RightOn( m_point[j], m_point[i], m_point[iMinus1] ) );
}
...
// Assumes this polygon is closed.
{
	...
	{
		P[i] = vrx*m_point[i].x+vry*m_point[i].y+vrz*m_point[i].z;
//		P[i] = m_point[i];
		P[i].ws=i;
	}
	...
	while (P.GetSize() > 3) {
		...
		for (i = 0; i < P.GetSize(); i++ ) {
			...
			if (P.Diagonal(i, iPlus2)) {
				(*TriPoly)[TriPoly->m_n++] = vrx*P[i].x+vry*P[i].y+vrz*P[i].z;
				(*TriPoly)[TriPoly->m_n-1].ws=P[i].ws;
				(*TriPoly)[TriPoly->m_n++] = vrx*P[iPlus1].x+vry*P[iPlus1].y+vrz*P[iPlus1].z;//P[iPlus1];
				(*TriPoly)[TriPoly->m_n-1].ws=P[iPlus1].ws;
				(*TriPoly)[TriPoly->m_n++] = vrx*P[iPlus2].x+vry*P[iPlus2].y+vrz*P[iPlus2].z;//P[iPlus2];
				(*TriPoly)[TriPoly->m_n-1].ws=P[iPlus2].ws;
...
BOOL CPolygon::CPTriangulate(CPolygon* TriPoly, CVector C)
...
BOOL CPolygon::PointIn(CVector P)
...
vector<CVector> CPolygon::getpoint()
{
 return m_point;
}
