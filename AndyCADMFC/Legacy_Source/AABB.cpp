...
//	int old=hdc->SetROP2(R2_NOTXORPEN);
	...
	hdc->MoveTo(corner(0));
	CPoint p0(corner(0));
	hdc->TextOut(p0.x,p0.y,CString("0"));
	
	hdc->LineTo(corner(1));
	CPoint p1(corner(1));
	hdc->TextOut(p1.x,p1.y,CString("1"));
	
	hdc->LineTo(corner(3));
	CPoint p3(corner(3));
	hdc->TextOut(p3.x,p3.y,CString("3"));
	
	hdc->LineTo(corner(2));
	CPoint p2(corner(2));
	hdc->TextOut(p2.x,p2.y, CString("2"));
	...
	hdc->LineTo(corner(4));
	CPoint p4(corner(4));
	hdc->TextOut(p4.x,p4.y, CString("4"));

	hdc->LineTo(corner(5));
	CPoint p5(corner(5));
	hdc->TextOut(p5.x,p5.y, CString("5"));

	hdc->MoveTo(corner(2));
	hdc->LineTo(corner(6));
	CPoint p6(corner(6));
	hdc->TextOut(p6.x,p6.y, CString("6"));

	hdc->LineTo(corner(7));
	CPoint p7(corner(7));
	hdc->TextOut(p7.x,p7.y, CString("7"));
