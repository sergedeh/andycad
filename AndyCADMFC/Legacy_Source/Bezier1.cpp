...
CBezier::CBezier(int degree,vector<CVector> mpoint)
{
	revol=false;
   ...
   Para2.resize(N+1);
    ...
    Knot_Univer(ncp+degree-1,degree,Kn,0,1);
 	...
 	INT_curve(mpoint,ncp,degree,Kn,Kn,CP);
 	ncp+=(degree-1);
	ffd=false;
...
void CBezier::drawselect(CDC *hdc)
{
	...
	hdc->MoveTo(ptsBegin); 
	hdc->Ellipse(CRect(CPoint(pb.x-5, pb.y+5),CPoint(pb.x+5, pb.y-5)));

	pitere=pitere+1;
	}
	hdc->MoveTo(ptsEnd); 
	hdc->Ellipse(CRect(CPoint(pe.x-5, pe.y+5),CPoint(pe.x+5, pe.y-5)));
	}
	else
	{
	...
	vector<CVector>::iterator bonbon;
		...
		vbonbon.y=pbonbon.y;
		vbonbon.z=0;
		hdc->Ellipse(CRect(CPoint(pbonbon.x-3, pbonbon.y+3),CPoint(pbonbon.x+3, pbonbon.y-3)));
		}
		CPoint pt0(core[8]);
		hdc->MoveTo(pt0);
...
void CBezier::addnode(CVector point, CDC *hdc)
{
	bool isOK=false;
...
{
	vector<CVector>::iterator rsult;
//	CP=(CVector*)realloc(CP,ContP.size()*sizeof(CVector));
	//ncp=ContP.size();
...
/*	int i=0;
	...
	//hdc->SetROP2(R2_MASKPEN);
	CRect v(p0,p2);
//	if(v.PtInRect(corepoint))
//	{
...
/*	   vector<CTriangle> l;
	   ...
	   {
		   CPoint pv1((*k)->vertex1->v);
		   CPoint pv2((*k)->vertex2->v);
		   CPoint pv3((*k)->vertex3->v);
...
/*	   vector<CRay> l;
	   ...
	   br.add(vb);
	   quadtree.query(&l,br);
	   for(vector<CRay>::iterator k=l.begin();k!=l.end();k++)
	   {
		   CPoint nb=k->getPhead();
		   CPoint nbplus=k->getPorg();
...
vector<CVector>* CBezier::translatep(CVector prevpos, CVector postpos)
{
	if(ffd)
	{
...
void CBezier::translate(CWnd *wnd,CDC *hdc,CVector prevpos, CVector postpos)
{
	...
	for (piterb=ptbegin.begin();piterb<ptbegin.end();piterb++)
	{
		...
		*piterb+=MouseVect;
		alr=true;
		redraw=true;
		if(!surf) drawTrans(hdc);
		}
		}
		
		else if ((pointinRegion(ptsEnd,prevpos))&&((!multiselect)||(!getLibstatus())))
		{
		multiselect=false;
		if(!ale)
		{
...
void CBezier::savefile(CFile &file)
{
	...
	for(it=0;it<=(ncp1+1)*(ncp+1);it++)
	{
		C[it].savefile(file);
	}
	for(it=0;it<ncp+deg+2;it++)
	{
		file.Write(&Kn[it], sizeof(double));
	}
	
	for(it=0;it<=ncp1+deg1+2;it++)
	{
		file.Write(&Kn1[it], sizeof(double));
	}
	
	for(it=0;it<=ncp2+deg2+2;it++)
	{
...
//	file.Write(&CP, sizeof(CVector)*ncp);
//	file.Write(&Kn, sizeof(Kn)*(ncp+deg+1));

	file.Write(&psize, sizeof(int));
	...
	for(vector<CVector>::iterator it=CP.begin();it<CP.end();it++)
	{
		file.Write(it, sizeof(CVector));
	}
	for(vector<double>::iterator its=Kn.begin();its<Kn.end();its++)
	{
		file.Write(its, sizeof(double));
	}
 */
/*	for (int i=0;i<ncp; i++)
	{
		file.Write(&CP[i],sizeof(CP[i])); 
	}
	
	for(int j=0; j<ncp+deg+1; j++)
	{
		 file.Write(&Kn[j],sizeof(Kn[j]));
	}
*/
...
void CBezier::addvec(vector<CVector> vectb, vector<CVector> vecte)
{
	...
	ptend=vecte;
//	free(CP);
//	CP=(CVector*)malloc(ptbegin.size());
	...
	case(L2):
		{
		    if (down)
			{
			/*	p=*(ContP.end()-2);/////DESACTIVER CETTE PARTIE POUR AVOIR UN sPLINE DrawPlus
				p=(p+pos)/2;
				ContP.insert(ContP.end()-1,p);
						ContP.push_back(pos);
				control1=pos;
				ContP.back()=pos;*/
				control2=pos;
...
//	Para_chord(0,1,ContP.begin(),ncp,Para.begin());
//	Grev_knot1(ncp,deg,Para);
	
	INT_curve(ContP,ncp,deg,Kn,Kn,CP);
//	INT_curveI(ContP.begin(),ncp,deg,Kn,Kn,CP.begin());
	ncp+=2;
/*	for(int j=0;j<4;j++)
	{
	P.push_back(CP[j]);
...
//	   M[iter]=insert_Boor(Para[iter],deg,Kn,CP,ncp);
	}
...
//   deg=3;           // degree of polynomial = t-1
 ...
 /*  Para=Para_universp(0,1,ncp+2);*/
//   Knot_deBoor(ncp,deg,Para,Kn);
 /*  Para=Para_universp(0,1,N);
   */      
...
/*
	CPen pen;
	pen.CreatePen(PS_SOLID,3,lcolor);
...
void CBezier::Knot_UniverSp(double min,double maxu, int n,int p,double *V)
{
	int m=n+p+1;
...
void CBezier::Bohms(double t, int p,vector<double>& U,vector<CVector>& P,int &n)
{
	...
	for(int i=0;i<=n+1;i++)
	{
		...
		if((k-p+1<=i)&&(i<=k))
		{
			Pv[i].x=P[i].x*((t-U[i])/(U[i+p]-U[i]))+P[i-1].x*((U[i+p]-t)/(U[i+p]-U[i]));
			Pv[i].y=P[i].y*((t-U[i])/(U[i+p]-U[i]))+P[i-1].y*((U[i+p]-t)/(U[i+p]-U[i]));
			Pv[i].z=P[i].z*((t-U[i])/(U[i+p]-U[i]))+P[i-1].z*((U[i+p]-t)/(U[i+p]-U[i]));
			Pv[i].w=P[i].w*((t-U[i])/(U[i+p]-U[i]))+P[i-1].w*((U[i+p]-t)/(U[i+p]-U[i]));
		}
...
//   Para=new double[N+1];
   ...
   for(int iter=0;iter<=N-1;iter=iter+1)
   {
	   CVector nb=NURBS(deg,CP,ncp,Para[iter],Kn);
	   CVector nbplus=NURBS(deg,CP,ncp,Para[iter+1],Kn);
	  ...
	  if ((pointtoVector(v,vpoint)<2)&&(v12<=v11))
	  {
		 // hitpoint=nb;
		   delete []M;
//		   delete []Para;
...
bool CBezier::snapok(CVector point)
{
	snap=false;
	snapb=false;
	pitere=ptend.begin();
	...
	for (piterb=ptbegin.begin();piterb<ptbegin.end();piterb++)
	{
				...
				if ((abs((piterb->x-point.x)<=10)&&(abs(piterb->y-point.y)<=10)))
				{
					snapvec=*piterb;
					snapb=true;
				}
				if ((abs((pitere->x-point.x)<=10)&&(abs(pitere->y-point.y)<=10)))
				{
					snapvec=*pitere;
					snapb=true;
				}
				if ((abs((pc.x-point.x)<=20)&&(abs(pc.y-point.y)<=20)))
				{
					snapvec=pc;
					snapb=true;
				}
				...
				{
				snapv.clear();
				snapv.push_back(*piterb);
				snapv.push_back(hitpoint);
				snapv.push_back(*pitere);
				snap=true;
				break;
				}
...
CBezier::CBezier(CFile &file)
{
   ...
   ncp=5;          // number of control points = n+1; for interpolation purpose add +3 to this number
                   // when calculating the knot vector
   deg=5;          // degree of polynomial = t-1
...
/*   int i=0;
  ...
  */
   ...
   for(int jiter=0;jiter<N1;jiter++)
   {
	 for(int iter=0;iter<N;iter++)
	 {
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*jiter+iter),mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*jiter+iter+1),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter));
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*jiter+iter+1),mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*(jiter+1)+iter+1),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter+1));
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*(jiter+1)+iter+1),mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*(jiter+1)+iter),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter+2));
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*(jiter+1)+iter),mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*jiter+iter),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter+3));
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*jiter+iter),mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*(jiter+1)+iter+1),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter+4));
		  i+=5;
	 }
   }
   if(N2==0) kiter++;
   ...
   int ed=i;
  	   for(i=0;i<ed;i+=5)
	   {
		  ...
		  mesh.Triangle(mesh.Tcount()-1)->edge3->flip();
   ...
   for(int kiter=0;kiter<N2;kiter++)
   {
	 ...
	 {
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*jiter+iter),mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*jiter+iter+1),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter));
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*jiter+iter+1),mesh.Vertex((N1+1)*(N+1)*(kiter+1)+(N+1)*jiter+iter+1),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter+1));
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*(kiter+1)+(N+1)*jiter+iter+1),mesh.Vertex((N1+1)*(N+1)*(kiter+1)+(N+1)*jiter+iter),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter+2));
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*(kiter+1)+(N+1)*jiter+iter),mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*jiter+iter),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter+3));
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*jiter+iter),mesh.Vertex((N1+1)*(N+1)*(kiter+1)+(N+1)*jiter+iter+1),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter+4));
		  i+=5;
	   }
   }
   if(N1==0) jiter++;
   ...
   for(int kiter=0;kiter<N2;kiter++)
   {
	   for(jiter=0;jiter<N1;jiter++)
	   {
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*jiter+iter),mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*(jiter+1)+iter),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter));
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*(jiter+1)+iter),mesh.Vertex((N1+1)*(N+1)*(kiter+1)+(N+1)*(jiter+1)+iter),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter+1));
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*(kiter+1)+(N+1)*(jiter+1)+iter),mesh.Vertex((N1+1)*(N+1)*(kiter+1)+(N+1)*jiter+iter),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter+2));
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*(kiter+1)+(N+1)*jiter+iter),mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*jiter+iter),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter+3));
		  mesh.addEdge(new CEdge(mesh.Vertex((N1+1)*(N+1)*kiter+(N+1)*jiter+iter),mesh.Vertex((N1+1)*(N+1)*(kiter+1)+(N+1)*(jiter+1)+iter),(N1+1)*(N+1)*kiter+(N+1)*jiter+iter+4));
		  i+=5;
	   }
   }
   if(N==0) iter++;
...
void CBezier::setFFDon(CString option)
{
	...
	if(FFDbox!=NULL) 
		...
		ptend.clear();
		   ...
		   {
				  ptbegin.push_back((*iv)->v);
				  ptend.push_back((*iv)->n);
		   }
		   ptend.push_back((*mesh.vbegin())->v);
			}
   	...
   	if(box.intersectAABB(FFDbox->box))
	{
		...
		else
		{
		   ...
		   for(iv=mesh.vbegin(),pitere=ptbegin.begin();iv!=mesh.vend();iv++,pitere++)
		   {
			  ...
			  {
				  (*iv)->updatev(NURBS(FFDbox->degu,FFDbox->degv,FFDbox->degw,Cb,FFDbox->ncpu,FFDbox->ncpv,FFDbox->ncpw,k.x,k.y,k.z,Knu,Knv,Knw));
				  *pitere=(*iv)->v;
...
//			ptend=mesh.vertexlist;
		   ...
		   for(iv=mesh.vbegin();iv!=mesh.vend();iv++)
		   {
			  if(FFDbox->box.containpts(**iv))//Mr[i]))
			  {
					  CVector k=FFDbox->box.getParam(**iv);
...
/*	mesh.updateShadingpara(loc,lit,mytext,pwidth,pheight,&l);*/
}
...
void CBezier::updatecore()
{
	...
	ptbegin.clear();
	ptend.clear();
	for(vector<CVertex*>::iterator i=mesh.vbegin();i!=mesh.vend();i++)
	{
		ptbegin.push_back((*i)->v);
	}
   ...
   int ed=i;
  	   for(i=0;i<ed;i+=5)
	   {
		  s->addTri(new CTriangle(s->Edge(i),s->Edge(i+1),s->Edge(i+4)));
  		  s->Triangle(s->Tcount()-1)->edge3->flip();
   ...
   ed=i;
	...
	if(ffd)
	{
		FFDbox->update();
		   ...
		   for(iv=mesh.vbegin();iv!=mesh.vend();iv++)
		   {
			  if(box.containpts(**iv))
			  {
					  CVector k=box.getParam(**iv);
					  FFDbox->Para[i]=k;
			  }
			  else
			  {
...
void CBezier::cap(bool vctop,bool vcbot,CDC* hdc)
{
...
//	   mesh.displayEdge(hdc);
	   ...
	   int ed=i;
  		   for(i=0;i<ed;i+=5)
		   {
			  ...
			  M[M.size()-1]->edge3->flip();

/*			  CVector v=t.getcent();
			  CRect r(v,v);
			  r.InflateRect(5,5,5,5);
			  if((t.vertex1->visible)&&(t.vertex2->visible)&&(t.vertex3->visible))
			  {
...
void CBezier::cap0(bool vctop,bool vcbot,CDC* hdc)
{
...
bool CBezier::clipTriangle(vector<CTriangle*>& Mb,CDC* hdc)
{
	CVector vrx,vry,vrz;
	vrx.getcoorx();vry.getcoory();vrz.getcoorz();
	int k=Mb.size();
 ...
 {
 CVector v1=*t->vertex1;
 CVector v2=*t->vertex2;
 CVector v3=*t->vertex3;
 CVector ve,vb;
 ...
 if(p1==CVector(0,0,0))
 {
	 Clipconfiguration0(&vb,&ve,t->vertex2,t->vertex3,t->vertex1,hdc);
 }else
 if(p2==CVector(0,0,0))
 {
	 Clipconfiguration0(&vb,&ve,t->vertex3,t->vertex2,t->vertex1,hdc);
 }else
 if(p0==CVector(0,0,0))
 {
	 Clipconfiguration0(&vb,&ve,t->vertex1,t->vertex3,t->vertex2,hdc);
 }else
 if(p0.dot(p1)<0)
 {
	 Clipconfiguration(&vb,&ve,t->vertex1,t->vertex2,t->vertex3,hdc);
 }else
 if(p0.dot(p2)<0)
 {
	 Clipconfiguration(&vb,&ve,t->vertex3,t->vertex1,t->vertex2,hdc);
 }else
 if(p1.dot(p2)<0)
 {
	 Clipconfiguration(&vb,&ve,t->vertex2,t->vertex3,t->vertex1,hdc);
 }
...
void CBezier::Clipconfiguration0(CVector* b,CVector* e,CVertex* vi0,CVertex* vi1,CVertex* vi2,CDC* hdc)
{
 CVector vrx,vry,vrz;
 vrx.getcoorx();vry.getcoory();vrz.getcoorz();
...
void CBezier::Clipconfiguration(CVector* b,CVector* e,CVertex* vi0,CVertex* vi1,CVertex* vi2,CDC* hdc)
{
