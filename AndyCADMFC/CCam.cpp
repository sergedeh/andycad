...
void CCAM::Translate(int sens,CVector prevpoint,CVector postpoint)
{
	...
	if(prevpoint!=postpoint)
	{
		...
		CGPoint po(postpoint);
		CGPoint pr(prevpoint);
		if(sens==0)
		{
			b=po.y-pr.y;
			vo.y=camor.y-b;
		...
		{
			CVector vb=CGPointv(po.x-pr.x,po.y-pr.y);
			vo=camor-vb;
		}
...
void CCAM::setCamview(string s,CAABB& box)
{
	CTVector tvec;
...
void CCAM::openfile(fstream &file)
{
	camor.openfile(file);
...
void CCAM::savefile(fstream &file)
{
	camor.savefile(file);
...
long CCAM::Readfile(fstream &file)
{
	camor.Readfile(file);
