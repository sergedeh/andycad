...
void CVertex::saveTo(ofstream& file)
{
	n.saveTo(file);
	v.saveTo(file);
	vc.saveTo(file);
...
void CVertex::loadFrom(ifstream& file)
{
	n.loadFrom(file);
	v.loadFrom(file);
	vc.loadFrom(file);
}
long CVertex::skipRead(ifstream& file)
{
...
{
	//CMesh::iedge vi=NULL;*how do you convert to null pointer?
