...
bool CVector::pointinRegion(CVector point, CVector region)
{
	...
	CPoint ppoint=point;
    int abs1=pregion.x-ppoint.x>0?pregion.x-ppoint.x:ppoint.x-pregion.x;
    int abs2=pregion.y-ppoint.y>0?pregion.y-ppoint.y:ppoint.y-pregion.y;
	if ((abs1<=5)&&(abs2<=5))
	{
		return true;
	}
...
std::string CVector::setplanesnap(std::string s)
{
	std::string sd;
	switch(planesnap)
	{
...
void CVector::saveTo(ofstream &file)
{
...
void CVector::loadFrom(ifstream &file)
{
...
long CVector::skipRead(ifstream &file)
{
