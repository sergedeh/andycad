...
void CTriangle::DrawTriangleFill(CDC *hdc,CVector loc[2],CCVector lit[6])
{
	...
	double x1=CGPoint(CVector(*edge1->Start())).x;
	double y1=CGPoint(CVector(*edge1->Start())).y;
	double x2=CGPoint(CVector(*edge2->Start())).x;
	double y2=CGPoint(CVector(*edge2->Start())).y;
	double x3=CGPoint(CVector(*edge3->Start())).x;
	double y3=CGPoint(CVector(*edge3->Start())).y;
...
void CTriangle::Draw_Triangle_2D(HDC hdc,BYTE* bits,int bheight,int bwidth,col* mytext,int pwidth,int pheight,light *l)
{
    ...
    if (vv1.p.y==vv2.p.y)
    {
        ...
        {
            //general triangle that ís needs to be broken up along long edge
            temp=vv2;
            ...
            Draw_Bottom_Tri(hdc,bits,bheight,bwidth,vv1,temp,vv2,mytext,pwidth,pheight,l);
            Draw_Top_Tri(hdc,bits,bheight,bwidth,vv2,temp,vv3,mytext,pwidth,pheight,l);
        }//end else
	SetMapMode(hdc,o);
