...
/*    CPen pen;
    ...
    else
    {
        ...
        {
            hdc->MoveTo(pb);
            CGPoint p1(prev.x,pb.y);
            hdc->LineTo(p1);
            
            hdc->LineTo(prev);
            CGPoint p2(pb.x,prev.y);
        ...
        CGPoint p1(pe.x,pb.y);
        hdc->LineTo(p1);
        hdc->LineTo(pe);
        
        CGPoint p2(pb.x,pe.y);
        hdc->LineTo(p2);
        hdc->LineTo(pb);
...
void CRectangle::createvec()
{
	...
	CGPoint pe=ptsEnd;
	CGPoint pb=ptsBegin;
	CGPoint p1=CGPointv(pe.x,pb.y);
	CGPoint p3= CGPointv(pb.x,pe.y);
...
void CRectangle::setType(string option)
{
...
bool CRectangle::getCharge()
{
    ...
    if (cancharge)
    {
        ...
        CGPoint p11(ptevir.back());
        CGPoint p22(ptbvir.back());
...
void CRectangle::savefile(fstream &file)
{
...
void CRectangle::openfile(fstream &file)
{
//    file.Read(&tprect, sizeof(TP));*************
    CLine::openfile(file);
    name="CRectangle";
}
long CRectangle::Readfile(fstream &file)
{
//    file.Seek(sizeof(TP),CFile::current);****************
    return CLine::Readfile(file);
}
CRectangle::CRectangle(fstream &file)
{
	OpClstate=close;
//	bcolor=NULL;*************
	cl=false;
