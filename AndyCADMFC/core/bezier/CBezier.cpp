...
void CBezier::OffsetMesh(float u, CFigure *f)
{
    ...
    for(int iter=0;iter<ncp-1;iter++)
    {
		...
		if(!surf)
		{
			if(select==pnt)
			{
				...
				for (piterb=ContP.begin();piterb<ContP.end();piterb++)
				{
					...
					hdc->MoveTo(ptsBegin);
					hdc->Ellipse(CRect(CGPoint(pb.x-15, pb.y+15),CGPoint(pb.x+15, pb.y-15)));
					CString sp;
					sp.Format(_T("%d"),io);
                    //		hdc->TextOut(ph.x,ph.y,sp);
					CGPoint ap(pb.x+35, pb.y+15);
		...
		if(!core.empty())
		{
			if(select==full)
			{
				for (bonbon=core.begin();bonbon<core.end()-3;bonbon++)
				{
					...
					if(*bonbon!=core[8])
					{
						hdc->Ellipse(CRect(CGPoint(pbonbon.x-13, pbonbon.y+13),CGPoint(pbonbon.x+13, pbonbon.y-13)));
					}
				}
				CGPoint pt0(rotateCenter);
				hdc->MoveTo(pt0);
				...
				hdc->Ellipse(CRect(CGPoint(pt0.x-13, pt0.y+13),CGPoint(pt0.x+13, pt0.y-13)));
				...
				hdc->Ellipse(CRect(CGPoint(pt1.x-17, pt1.y+17),CGPoint(pt1.x+17, pt1.y-17)));
...
bool CBezier::PtinRegion(CVector point)
{
	...
	if(surf)
	{
        ...
        {
            CGPoint pv1((*k)->Vertex(0)->v);
            CGPoint pv2((*k)->Vertex(1)->v);
            CGPoint pv3((*k)->Vertex(2)->v);
...
/*            int s=0;
            ...
            CGPoint *Pt=new CGPoint[(N+1)*(N1+1)];
            CGPoint *Pt1=new CGPoint[(N+1)*(N1+1)];
            lk=Kn[deg]*N;
            ...
            for(int jiter=0;jiter<=N1;jiter++)
            {
                ...
                {
                    CGPoint nb=(CVector)*mesh.Vertex((N+1)*jiter+(iter-lk));
                    CGPoint nbplus=(CVector)*mesh.Vertex((N+1)*jiter+(iter+1-lk));
                    
                    
                    CGPoint vpo=CGPoint(point)-nb;
                    CVector vpoint(vpo.x,vpo.y);//nb,point);
                    CGPoint vppo=nbplus-CGPoint(nb);
                    CVector v(vppo.x,vppo.y);//nb,nbplus);
                    CGPoint v2=nb+nbplus,v22;
                    v2.x=v2.x/2;
                    v2.y=v2.y/2;
                    v22=v2;
                    v2=v2-nb;
                    ...
                    vpo=CGPoint(point)-nb;
                    CVector vpoint1(vpo.x,vpo.y);//nb,point);
                    vppo=nbplus-CGPoint(nb);
...
bool CBezier::inRegion(CVector point)
{
  ...
  /*      for(int iter=0;iter<N;iter++)
        {
            CGPoint nb=NURBS(deg,CP,ncp,Para[iter],Kn);
            CGPoint nbplus=NURBS(deg,CP,ncp,Para[iter+1],Kn);
...
*///*******************
void CBezier::savefile(fstream &file)
{
 /*
...
void CBezier::openfile(fstream &file)
{
 /*   //	init();
...
long CBezier::Readfile(fstream &file)
{
/*	init();
	LONG g;
	file.Read(&N, sizeof(N));
...
void CBezier::setPtsBegin(CGPoint pos)
{
	down=true;
	polystate(pos);
...
/*    if(redraw)
    {
        ...
        if(iv!=NULL){
            for(int iter=0;iter<N;iter++)
            {
                Mr[iter]=NURBS(deg,CP,ncp+1,Para[iter],Kn);
                (iv)->second->v=Mr[iter];
                iv++;
            }
        }
...
void CBezier::insertknot(double t, int h, int p,vector<double> U,vector<CVector> P,int n)
{
	...
	if((h+s)<=p+1)
	{
        ...
        for(int iter=k-p+h+1;iter<=k-s;iter++)
        {
...
void CBezier::insertknotU(double t, int h, int p,vector<double> U,int n)
{
	...
	if (s!=0)
	{
        ...
        for(int iter=k+1;iter<=k+h;iter++)
        {
            Uc[iter]=t;
            
        }
        for(int iter=k+h+1;iter<=m+h;iter++)
        {
            Uc[iter]=U[iter-h];
            
        }
        for(int iter=0;iter<=m+h;iter++)
        {
            U[iter]=Uc[iter];
...
void CBezier::Bohms(double t, int p,vector<double>& U,vector<CVector>& P,int &n)
{
	...
	for(int iter=k+2;iter<=m+1;iter++)
	{
	...
	for(int iter=0;iter<=n+p+1;iter++)
	{
		U[iter]=Uc[iter];
...
//DEL }
bool CBezier::fsnap(CVector& v,CVector &vl,string snapmode,CDC* hdc)
{
...
/*void CBezier::surface(CDC *hdc)
 ...
 Ptr=new CGPoint [(N1+1)*(lh-lk+1)];
 Pt1r=new CGPoint [(N1+1)*(lh-lk+1)];
...
void CBezier::volume(CDC *hdc)
{
    ...
    Ptr=new CGPoint [(N2+1)*(N1+1)*(lh-lk+1)];
    Pt1r=new CGPoint [(N2+1)*(N1+1)*(lh-lk+1)];
...
void CBezier::surface(CBezier *fig1, CFigure *fig2)
{
	...
	for(int i=0;i<=fig1->Para.size();i++)
	{
		Para[i]=fig1->Para[i];
	}
...
void CBezier::surface(string type,vector<CBezier*> fg, int l,float vthick)
{
	if(type=="loft")
	{
		loftsurface(fg,l);
...
/*
 ...
 CGPoint tl(CVector(xleft,ytop,0));
 CGPoint lb(CVector(xright,ybot,0));
 hdc->LPtoDP(&tl);
 hdc->LPtoDP(&lb);
 xleft=tl.x;
 ytop=tl.y;
 ...
 //	vl.push_back(e);//	insert/remove edges from ìactive edge listî
...
void CBezier::ScanPolygon(CDC *hdc, COLORREF rgba,CVector *K,int s,CImagex *texture)
{
    /*	int pheight;
     ...
     CGPoint blend(blend_p);
     hdc->LPtoDP(&blend);
     blend.x=xleft;
     blend.y=ytop;
     CGPoint nblend(blend_plane);
     hdc->LPtoDP(&nblend);
     nblend.x=width;
     nblend.y=0;
     blend_radius=width;
     ...
     //	insert/remove edges from ìactive edge listî
     if(mode_texture!=wrap)
     {
     for(vector<Edges>::iterator ip=vl.begin();ip!=vl.end();ip++)
     updatex(ip,y,width,xleft);//update x coordinate of every active edge
...
void CBezier::setFFDon(string option)
{
...
void CBezier::Clipconfiguration0(CVector* b,CVector* e,CVertex* vi0,CVertex* vi1,CVertex* vi2)
{
    ...
    CRay v(*b,*e,1);
    CVector no=CVector(*vi1);
    CVector n1=CVector(*vi2);
    CRay bi(no,n1,1);
...
void CBezier::Clipconfiguration(CVector* b,CVector* e,CVertex* vi0,CVertex* vi1,CVertex* vi2)
{
    ...
    CRay vo(*b,*e,1);
    CVector no=CVector(*vi0);
    CVector n1=CVector(*vi1);
    CRay bi(no,n1,1);
