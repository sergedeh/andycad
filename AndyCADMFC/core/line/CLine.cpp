...
void CLine::savefile(fstream &file)
{
	CFigure::savefile(file);
    
}

void CLine::openfile(fstream &file)
{
...
long CLine::Readfile(fstream &file)
{
	return CFigure::Readfile(file);
    
}

CLine::CLine(fstream &file)
{
	lstate=L1; move=false;down=false;Up=false;;UfromL2=false;
	CVector v0(0,0);
	precEnd=v0;menusel=false;
...
void CLine::Fill (CDC*hdc,CGPoint p , COLORREF newVal)
{
/*    CGPoint h=p;
    hdc->LPtoDP(&p);
...
{
    CVector no=CVector(0,y,0);
    CVector n1=CVector(1,y,0);
	CRay v1(no,n1);
...
bool isxleft(CGPoint e1,CGPoint e2)
{
	return e1.x<e2.x;
}
...
float CLine::computealpha(int x, int y,CPlane *pl,float init,float end)
{
	...
	if(alpha<0)
	{
        ...
        {
            alpha=-alpha;
            return min(init+alpha/blend_radius,(double)end);
        }
	}
	else
	{
...
void CLine::SendNotify(string s, void *param[7])
{
