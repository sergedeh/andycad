...
CDimL::CDimL(fstream &file,CDC* hdc)
{
	name="CDimL";
	state=false;
	openfile(file,hdc);
...
void CDimL::savefile(fstream &file)
{
	//file.Write(&style,sizeof(STYLE));********************
	CLibS::savefile(file);
}

void CDimL::openfile(fstream &file,CDC* hdc)
{
...
long CDimL::ReadFike(fstream &file)
{
