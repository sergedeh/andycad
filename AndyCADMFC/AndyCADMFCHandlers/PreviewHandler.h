...
#include <afxext.h>
#include "..\AndyCADMFC\AndyCADMFCDoc.h"
#include "..\AndyCADMFC\AndyCADMFCView.h"
...
protected:
	...
	DECLARE_DOCUMENT(CAndyCADMFCDoc)
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(CAndyCADMFCView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(CAndyCADMFCDoc, (CObject*) m_pDocument->GetContainer());
