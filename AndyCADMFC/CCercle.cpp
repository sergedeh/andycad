...
bool CCercle::DrawEllipse(bool fPrevB,CDC *hdc, CVector p)
{
	...
	if(it>1)
	{
        ...
        /*	 CVector b1,b2,bi,ba,l1,l2;
         ...
         CGPoint p1l(p1.x,p1.y),p2l(p2.x,p2.y),p3l(p3.x,p3.y),p4l(p4.x,p4.y),p5l(p5.x,p5.y),p6l(p6.x,p6.y),p7l(p7.x,p7.y),p8l(p8.x,p8.y);
...
bool CCercle::DrawCircle(bool fPrevB,CDC *hdc, CVector p)
{
    ...
    CGPoint ppt(pt);
    CGPoint pr=CGPointv(pCt.x-ppt.x,pCt.y-ppt.y);
    CVector vpCt(pCt.x,pCt.y);
    CVector vpr(pr.x,pr.y);
...
bool CCercle::DrawArc(bool fPrevB,CDC *hdc, CVector p)
{
	...
	if(it>1)
	{
        ...
        /*	   control2.snaptoplane(control2);
         ...
         control2.snaptoplane(pt);
         */
        string fg=control1.setplanesnap("front");
        ...
        for(int jo=0;jo<ncp;jo++)
        {
            if(jo%3==0)
            {
                auto n=find(vcenter.begin(),vcenter.end(),CP[jo]);
                if(jo+1<d)
                {
                    CVector vp=CP[jo+1]-CP[jo];
                    CP[jo+1]=CP[jo]+polarc(a*absc(r),argu(vp));
...
bool CCercle::Draw3DotArc(bool fPrevB,CDC *hdc, CVector p)
{
	...
	if(it>1)
	{
        ...
        string fg=b1.setplanesnap("front");
 ...
 */
void CCercle::updateCP(string s)
{
...
void CCercle::setType(string type)
{
	ntype=type;
}
...
bool CCercle::PtinRegion(CVector point)
{
    ...
    if(!surf)
    {
        ...
        else
        {
            ...
            for(int iter=0;iter<N;iter++)
            {
                CGPoint nb=Mr[iter];//NURBS(deg,CP,ncp,Para[iter],Kn);
                CGPoint nbplus=Mr[iter+1];//NURBS(deg,CP,ncp,Para[iter+1],Kn);
...
void CCercle::savefile(fstream &file)
{
...
void CCercle::openfile(fstream &file)
{
...
long CCercle::Readfile(fstream &file)
{
